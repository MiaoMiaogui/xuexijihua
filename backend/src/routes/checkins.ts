...
const CHECKIN_TYPES = ['one_tap', 'photo', 'duration'] as const;

router.post('/', auth, async (req, res) => {
  try {
    const { taskId, subjectId, type, durationMin } = req.body || {};
    if (typeof type !== 'string' || !CHECKIN_TYPES.includes(type as any)) {
      return res.status(400).json({ code: 400, message: '无效的打卡类型' });
    }
    ...
    console.error('[checkins] POST failed:', e?.message || e);
    res.status(503).json({ code: 503, message: '打卡失败，请稍后重试' });
  }
});
...
router.get('/streak', auth, async (req, res) => {
  ...
  const set = new Set(rows.map((r) => dayjs(r.check_date).format('YYYY-MM-DD')));
  let streak = 0;
  let cursor = dayjs();
